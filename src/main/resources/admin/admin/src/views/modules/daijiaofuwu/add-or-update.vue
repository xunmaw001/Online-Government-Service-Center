		...
		>
			<template >
				...
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="缴费编号" prop="jiaofeibianhao">
					<el-input v-model="ruleForm.jiaofeibianhao" placeholder="缴费编号" clearable  :readonly="ro.jiaofeibianhao"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"50%","margin":"0 0 20px 0"}' v-else class="input" label="缴费编号" prop="jiaofeibianhao">
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `daijiaofuwu/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.daijiaofuwuCrossAddOrUpdateFlag = false;
						 this.parent.search();
